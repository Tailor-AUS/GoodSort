# Email to Tomra / Cleanaway — Depot Drop-off Agreement

**To:** [Tomra Yeerongpilly or Salisbury depot — find contact via tomra.com/collection-points or call ahead]
**From:** Knox Hart (knox.hart@gmail.com)
**Subject:** Volume Drop-off Agreement — The Good Sort (Residential CDS Collection, Brisbane South)

---

Hi,

My name is Knox Hart and I run **The Good Sort** — a scan-first suburb volume collection we are standing up in Brisbane's inner south (Moorooka, South Brisbane, West End).

When suburb volume unlocks a run, we bring bagged CDS-eligible containers to your depot. Your Yeerongpilly / Salisbury facility is the closest to that first area.

## What we expect to deliver

- Pre-separated material in 3 streams: **aluminium cans, PET bottles, glass bottles**
- Delivered by our runners in bags or tubs (not mixed / not compacted)
- Each delivery accompanied by a household manifest (scan is optional verification, not required for every container)
- Starting volume is not live yet — we will confirm a first-drop window once suburbs unlock. Planning range: **200–500 containers/week**, then **2,000–5,000/week** if density holds

## What we need

1. **Confirmation we can drop off as a bulk/volume account** — not as a walk-in consumer
2. **Counting/verification process** — do you hand-count or weight-estimate? Can we pre-count digitally?
3. **Payment terms** — 10c per eligible container, less your handling fee. What is the fee structure?
4. **Operating hours for drop-off** — we'd typically deliver Monday–Friday mornings
5. **Any paperwork** — ABN, insurance, registration forms we need to fill out

## Our details

- **Entity:** Tailor Intelligence Pty Ltd (ABN 13 688 827 572), trading as The Good Sort
- **Website:** [thegoodsort.org](https://www.thegoodsort.org)
- **Contact:** Knox Hart — knox.hart@gmail.com — 0400 XXX XXX [add your mobile]

Happy to come by in person to discuss. I'm based in Moorooka, 10 minutes away.

Thanks,
Knox
